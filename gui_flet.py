...
def create_card(item, page):
    def on_click(e):
        ...
        if isinstance(kd, dict):
            kd_text = kd.get('text', '-')
            kd_image = kd.get('link')
            kd_link = kd.get('link')
            details.append(ft.Text(f"Кд: {kd_text}", selectable=True))
            if kd_image:
                if kd_image.startswith("http://") or kd_image.startswith("https://"):
                    print(f"Добавление изображения КД с URL: {kd_image}")
        ...
        if isinstance(notes, dict):
            ...
            notes_image = notes.get('link')
            details.append(ft.Text(f"Примечания: {notes_text}", selectable=True))
            if notes_image:
                if notes_image.startswith("http://") or notes_image.startswith("https://"):
                    print(f"Добавление изображения из примечаний с URL: {notes_image}")
    ...
    if isinstance(bot_info, dict):
        image_path = bot_info.get('link')
        if image_path and (image_path.startswith("http://") or image_path.startswith("https://")):
            print(f"Добавление аватарки с URL: {image_path}")

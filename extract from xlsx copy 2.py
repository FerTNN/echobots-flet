...
class ExcelToJsonConverter:
    ...
    def _get_cell_hyperlink(self, worksheet, row, col):
        """Extract hyperlink from Excel cell"""
        try:
            cell = worksheet.cell(row=row+1, col=col+1)  # Excel uses 1-based indexing
            if cell.hyperlink:
                return cell.hyperlink.target
        except Exception as e:
            print(f"Error getting hyperlink from cell: {e}")
        return None
